    ...
    if (!queue) {
        ...
        try {
            ...
            queue = player.nodes.create(guild, {
                ...
                repeatMode: QueueRepeatMode[playerOptions.loopMode] as unknown as QueueRepeatMode,
    ...
    try {
        ...
        await queue.metadata.channel.send({
            embeds: [EmbedGenerator.Success({
                title: `${result.searchResult.hasPlaylist() ? 'Playlist' : 'Track'} queued!`,
                description: `The ${result.searchResult.hasPlaylist() ? 'playlist' : 'track'
                    } **${result.searchResult.playlist?.title || result.searchResult.tracks[0].title
