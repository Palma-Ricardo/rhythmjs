...
export async function SkipAction(
    ...
    if (!back) {
        ...
        return await queue.metadata.channel.send({
            embeds: [EmbedGenerator.Success({
                title: 'Track skipped!',
                description: `Track was skipped to the next track from the website by ${info.displayName} <@${info.id}>).`,
            })]
        });
    }
    ...
    await queue.metadata.channel.send({
        embeds: [EmbedGenerator.Success({
            title: 'Track skipped!',
            description: `The track was skipped to the previous track by ${info.displayName} (<@${info.id}>).`,
        })]
    });
}
