...
export async function createSocketServer(server: Server) {
    ...
    io.on('connection', async (socket) => {
        ...
        socket.on('equalizer', (eq: EqualizerBand[]) => EqualizerAction(user, socket, eq));
        socket.on('shuffle', (shuffle: boolean) => ShuffleAction(user, socket, shuffle));
        ...
        const handler = () => {
            ...
            if (queue?.connection) {
                socket.emit('statistics', {
                    ...
                    listeners: queue.channel?.members.filter((mem) => !mem.user.bot).size ?? 0,
